...
struct Triangle
{
    ...
    public Triangle(TriangleProjection triangle, byte red, byte green, byte blue)
    {
        triangle.A = A;
        Red = red;
        Green = green;
...
class Graphic
{
    public Graphic(WriteableBitmap bitmap, nint startPtr)
    {
        Bitmap = bitmap;
        StartPtr = startPtr;
    }
    ...
    public unsafe void DrawRectangle(Rectangle rectangle, byte r, byte g, byte b)
    {
        ...
        {
            for (int* ptr = (int*)(StartPtr + Bitmap.BackBufferStride * y) + rectangle.X1, eptr = ptr - rectangle.X1 + rectangle.X2;
             ptr != eptr;
              ++ptr)
            {
                *ptr = color;
    ...
    public unsafe void DrawTopTriangle(Int32Point basePoint, double delta1, double delta2, int triangleHeight, byte r, byte g, byte b)
    {
        ...
        for (int y = basePoint.Y; y <= basePoint.Y + triangleHeight; y++)
        {
            ...
            {
                var ptr = x * 4 + StartPtr + Bitmap.BackBufferStride * y;
                *(int*)ptr = color;
            }
            rowStart += delta1;
            rowEnd += delta2;
    ...
    public unsafe void DrawJuliaSet(int angle)
    {
        var complex = new Complex(Sin(angle), Cos(angle));
        for (int column = 0; column < 1000; column++)
        {
            for (int row = 0; row < 1000; row++)
            {
                var a = new Complex(column / 500 - 1 / 1000, row / 500 - 1 / 1000);
                for (int i = 0; i < 256; i++)
                {
                    a = a * a + complex;
                    if (a.Magnitude > 10)
    ...
    {
        NativeMemory.Fill((void*)(StartPtr), (nuint)(Bitmap.BackBufferStride * Bitmap.PixelHeight), 0);
    }
